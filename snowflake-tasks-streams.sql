...
-- ============================================
-- Architecture: RAW → STAGING → PRODUCTION
-- Traitement automatisé avec CDC (Change Data Capture)

USE DATABASE CAVES_ALBERT_DB;
USE SCHEMA RAW_DATA;

-- ============================================
-- 1️⃣ CRÉATION DES TABLES DE STAGING ET PRODUCTION
-- ============================================
...
-- Table de production pour l'inventaire en temps réel
...
-- ============================================
-- 2️⃣ CRÉATION DES STREAMS (CDC)
-- ============================================

-- Stream sur la table RAW pour détecter les nouvelles commandes
CREATE OR REPLACE STREAM STREAM_RAW_ORDERS
ON TABLE RAW_EVENTS_STREAM
APPEND_ONLY = TRUE
COMMENT = '🛒 Stream pour capturer les événements ORDER_CREATED en temps réel';

-- Stream sur la table RAW pour détecter les ajustements d'inventaire
CREATE OR REPLACE STREAM STREAM_RAW_INVENTORY
ON TABLE RAW_EVENTS_STREAM
APPEND_ONLY = TRUE
COMMENT = '📦 Stream pour capturer les événements INVENTORY_ADJUSTED en temps réel';

-- Stream sur la table de staging des commandes pour détecter les changements
CREATE OR REPLACE STREAM STREAM_STG_ORDERS
ON TABLE STAGING.STG_ORDERS
COMMENT = '🔄 Stream pour propager les commandes staging → production';

-- Stream sur la table de staging des ajustements pour détecter les changements
CREATE OR REPLACE STREAM STREAM_STG_INVENTORY
ON TABLE STAGING.STG_INVENTORY_ADJUSTMENTS
COMMENT = '🔄 Stream pour propager les ajustements staging → production';

-- ============================================
-- 3️⃣ CRÉATION DES TASKS
-- ============================================

-- TASK 1: RAW → STAGING (Commandes)
CREATE OR REPLACE TASK TASK_RAW_TO_STAGING_ORDERS
    WAREHOUSE = COMPUTE_WH
    SCHEDULE = '1 MINUTE'
    COMMENT = '⏱️ Extrait les commandes de RAW vers STAGING toutes les 1 minute'
WHEN
...
-- TASK 2: RAW → STAGING (Inventaire)
CREATE OR REPLACE TASK TASK_RAW_TO_STAGING_INVENTORY
    WAREHOUSE = COMPUTE_WH
    SCHEDULE = '1 MINUTE'
    COMMENT = '⏱️ Extrait les ajustements d''inventaire de RAW vers STAGING toutes les 1 minute'
WHEN
...
-- TASK 3: STAGING → PRODUCTION (Commandes)
CREATE OR REPLACE TASK TASK_STAGING_TO_PROD_ORDERS
    WAREHOUSE = COMPUTE_WH
    AFTER TASK_RAW_TO_STAGING_ORDERS
    COMMENT = '🚀 Charge les commandes de STAGING vers PRODUCTION (exécuté après TASK_RAW_TO_STAGING_ORDERS)'
WHEN
...
-- TASK 4: STAGING → PRODUCTION (Inventaire - Historique)
CREATE OR REPLACE TASK TASK_STAGING_TO_PROD_INVENTORY_HISTORY
    WAREHOUSE = COMPUTE_WH
    AFTER TASK_RAW_TO_STAGING_INVENTORY
    COMMENT = '📚 Charge l''historique des ajustements d''inventaire vers PRODUCTION'
WHEN
...
-- TASK 5: STAGING → PRODUCTION (Inventaire - État Actuel)
CREATE OR REPLACE TASK TASK_STAGING_TO_PROD_INVENTORY_CURRENT
    WAREHOUSE = COMPUTE_WH
    AFTER TASK_STAGING_TO_PROD_INVENTORY_HISTORY
    COMMENT = '📊 Met à jour l''état actuel de l''inventaire en temps réel'
WHEN
    SYSTEM$STREAM_HAS_DATA('STREAM_STG_INVENTORY')
AS
...
-- ============================================
-- 4️⃣ ACTIVATION DES TASKS (DAG)
-- ============================================
-- Les tasks s'exécutent dans l'ordre suivant :
-- 1. TASK_RAW_TO_STAGING_ORDERS (toutes les 1 min)
--    └── 2. TASK_STAGING_TO_PROD_ORDERS (après task 1)
-- 3. TASK_RAW_TO_STAGING_INVENTORY (toutes les 1 min)
--    └── 4. TASK_STAGING_TO_PROD_INVENTORY_HISTORY (après task 3)
--        └── 5. TASK_STAGING_TO_PROD_INVENTORY_CURRENT (après task 4)
...
-- ============================================
-- 5️⃣ MONITORING ET GESTION
-- ============================================

-- Vérifier l'état des tasks
SHOW TASKS IN SCHEMA RAW_DATA;

-- Voir l'historique d'exécution des tasks
SELECT
    NAME,
    STATE,
    SCHEDULED_TIME,
...
-- Vérifier si les streams ont des données en attente
...
-- Exécuter manuellement une task (pour tester)
-- EXECUTE TASK TASK_RAW_TO_STAGING_ORDERS;

-- Supprimer les tasks (si besoin de tout recréer)
...
-- Alertes : Produits avec stock faible (< 50 unités)
SELECT
    PRODUCT_ID,
    PRODUCT_NAME,
    PRODUCT_CATEGORY,
